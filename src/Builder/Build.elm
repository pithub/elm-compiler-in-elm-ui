...
type alias State a e f g h =
  Details.State a (LocalState a e f g h) e f g h


type LocalState a e f g h = LocalState
  {- mvStatus -} (MVar.State (State a e f g h) Status)
  {- mvStatusMap -} (MVar.State (State a e f g h) (Map.Map ModuleName.Raw (MVar Status)))
  {- mvRootStatus -} (MVar.State (State a e f g h) RootStatus)
  {- mvRootResult -} (MVar.State (State a e f g h) RootResult)
  {- mvResult -} (MVar.State (State a e f g h) Result)
  {- mvResultMap -} (MVar.State (State a e f g h) ResultDict)
  {- mvCachedInterface -} (MVar.State (State a e f g h) CachedInterface)


initialState : LocalState a e f g h
...
type alias IO a e f g h v =
  IO.IO (State a e f g h) v
...
makeEnv : FilePath -> Details.Details -> IO a e f g h Env
...
toAbsoluteSrcDir : FilePath -> Outline.SrcDir -> IO a e f g h AbsoluteSrcDir
...
--
fork : MVar.Lens (State a e f g h) v -> (() -> IO a e f g h v) -> IO a e f g h (MVar v)
fork = MVar.newWaiting


forkWithKey : MVar.Lens (State a e f g h) w -> (comparable -> v -> () -> IO a e f g h w) -> Map.Map comparable v -> IO a e f g h (Map.Map comparable (MVar w))
...
fromExposed : FilePath -> Details.Details -> DocsGoal a e f g h docs -> NE.TList ModuleName.Raw -> IO a e f g h (Either Exit.BuildProblem docs)
...
fromPaths : FilePath -> Details.Details -> NE.TList FilePath -> IO a e f g h (Either Exit.BuildProblem Artifacts)
...
crawlDeps : Env -> MVar StatusDict -> TList ModuleName.Raw -> v -> IO a e f g h v
...
crawlFile : Env -> MVar StatusDict -> DocsNeed -> ModuleName.Raw -> FilePath -> File.Time -> Details.BuildID -> IO a e f g h Status
...
checkModule : Env -> Dependencies -> MVar ResultDict -> ModuleName.Raw -> Status -> () -> IO a e f g h Result
...
checkDeps : FilePath -> ResultDict -> TList ModuleName.Raw -> Details.BuildID -> IO a e f g h DepsStatus
...
checkDepsHelp : FilePath -> ResultDict -> TList ModuleName.Raw -> TList Dep -> TList Dep -> TList CDep -> TList (ModuleName.Raw,Import.Problem) -> Bool -> Details.BuildID -> Details.BuildID -> IO a e f g h DepsStatus
...
loadInterfaces : FilePath -> TList Dep -> TList CDep -> IO a e f g h (Maybe (Map.Map ModuleName.Raw I.Interface))
...
checkMidpoint : MVar (Maybe Dependencies) -> Map.Map ModuleName.Raw Status -> IO a e f g h (Either Exit.BuildProjectProblem Dependencies)
...
checkMidpointAndRoots : MVar (Maybe Dependencies) -> Map.Map ModuleName.Raw Status -> NE.TList RootStatus -> IO a e f g h (Either Exit.BuildProjectProblem Dependencies)
...
compile : Env -> DocsNeed -> Details.Local -> String -> Map.Map ModuleName.Raw I.Interface -> Src.Module -> IO a e f g h Result
...
writeDetails : FilePath -> Details.Details -> Map.Map ModuleName.Raw Result -> IO a e f g h ()
...
finalizeExposed : FilePath -> DocsGoal a e f g h docs -> NE.TList ModuleName.Raw -> Map.Map ModuleName.Raw Result -> IO a e f g h (Either Exit.BuildProblem docs)
finalizeExposed root docsGoal exposed results =
...
type alias DocsGoal a e f g h v =
  { kind : DocsGoalKind
  , finalize : Map.Map ModuleName.Raw Result -> IO a e f g h v
  }
...
keepDocs : DocsGoal a e f g h Docs.Documentation
keepDocs =
  { kind = KeepDocs
  , finalize = finalizeKeepDocs
  }

writeDocs : FilePath -> DocsGoal a e f g h ()
writeDocs path =
  { kind = WriteDocs
...
toDocsNeed : DocsGoal a e f g h v -> DocsNeed
toDocsNeed goal =
  case goal.kind of
...
finalizeDocs : DocsGoal a e f g h docs -> Map.Map ModuleName.Raw Result -> IO a e f g h docs
finalizeDocs goal results =
  goal.finalize results

finalizeKeepDocs : Map.Map ModuleName.Raw Result -> IO a e f g h Docs.Documentation
finalizeKeepDocs results =
  IO.return <| Map.mapMaybe toDocs results

finalizeWriteDocs : FilePath -> Map.Map ModuleName.Raw Result -> IO a e f g h ()
finalizeWriteDocs path results =
  E.writeUgly path <| Docs.encode <| Map.mapMaybe toDocs results

finalizeIgnoreDocs : Map.Map ModuleName.Raw Result -> IO a e f g h ()
...
findRoots : Env -> NE.TList FilePath -> IO a e f g h (Either Exit.BuildProjectProblem (NE.TList RootLocation))
findRoots env paths =
...
getRootInfo : Env -> FilePath -> IO a e f g h (Either Exit.BuildProjectProblem RootInfo)
...
getRootInfoHelp : Env -> FilePath -> FilePath -> IO a e f g h (Either Exit.BuildProjectProblem RootInfo)
...
isInsideSrcDirByName : TList FileName -> AbsoluteSrcDir -> IO a e f g h Bool
...
crawlRoot : Env -> MVar StatusDict -> RootLocation -> () -> IO a e f g h RootStatus
...
checkRoot : Env -> ResultDict -> RootStatus -> () -> IO a e f g h RootResult
...
fromRepl : FilePath -> Details.Details -> String -> IO a e f g h (Either Exit.Repl ReplArtifacts)
...
finalizeReplArtifacts : Env -> String -> Src.Module -> DepsStatus -> ResultDict -> Map.Map ModuleName.Raw Result -> IO a e f g h (Either Exit.Repl ReplArtifacts)
...
{- NEW: findModulePath -}
findModulePath : FilePath -> Details.Details -> ModuleName.Raw -> IO a e f g h (Maybe FilePath)
