...
canonicalize env (A.At typeRegion tipe) =
    case tipe of
        ...
        Src.TType region name args ->
            MResult.bind (Env.findType region env name) <|
                canonicalizeType env typeRegion name args

        Src.TTypeQual region home name args ->
            MResult.bind (Env.findTypeQual region env home name) <|
                canonicalizeType env typeRegion name args

        Src.TLambda a b ->
            MResult.ok Can.TLambda
                |> MResult.andMap (canonicalize env a)
                |> MResult.andMap (canonicalize env b)

        Src.TRecord fields ext ->
            MResult.bind (Dups.checkFields (canonicalizeFields env fields)) <|
                \dsr ->
                    MResult.bind (MResult.sequenceAMap dsr) <|
                        \cfields ->
                            MResult.pure <| Can.TRecord cfields (Maybe.map A.toValue ext)

        Src.TUnit ->
            MResult.ok Can.TUnit

        Src.TTuple a b cs ->
            MResult.ok Can.TTuple
                |> MResult.andMap (canonicalize env a)
                |> MResult.andMap (canonicalize env b)
                |> MResult.andMap
                    (case cs of
