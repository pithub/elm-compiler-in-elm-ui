...
check : Json.TString -> Either (TList Json.TString) License
check givenCode =
  if Map.member givenCode osiApprovedSpdxLicenses then
    ...
    let
      pairs =
        MList.map (\code -> (code, Json.toChars code)) (Map.keys osiApprovedSpdxLicenses)
        ++
        Map.toList osiApprovedSpdxLicenses
    in
    Left <| MList.map Tuple.first <| MList.take 4 <|
      Suggest.sort givenCode Tuple.second pairs
...
--
osiApprovedSpdxLicenses : Map.Map Json.TString String
