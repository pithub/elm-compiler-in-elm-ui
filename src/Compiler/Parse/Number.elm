...
chompZero src pos end =
    if pos >= end then
        ...
        if word == 0x78 {- x -} then
            ...
            CErr pos E.NumberNoLeadingZero
...
stepHex word acc =
    if 0x30 {- 0 -} <= word && word <= 0x39 {- 9 -} then
        ...
        16 * acc + 10 + (word - 0x61 {- a -})

    else if 0x41 {- A -} <= word && word <= 0x46 {- F -} then
        16 * acc + 10 + (word - 0x41 {- A -})
